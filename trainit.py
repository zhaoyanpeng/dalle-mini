...
from src import dalle_mini
from src.dalle_mini.data import Dataset
from src.dalle_mini.model import (
    DalleBart,
    DalleBartConfig,
    DalleBartTokenizer,
    set_partitions,
)
from tools.train.scalable_shampoo.distributed_shampoo import (
    GraftingType,
    distributed_shampoo,
)
...
class ModelArguments:
    ...
    def __post_init__(self):
        ...
        if self.restore_state:
            assert self.model_name_or_path is not None and (
                ...
                or os.path.isdir(self.model_name_or_path)
            ), "Restoring state only available with W&B artifact reference"

    def get_metadata_from_local_wandb(self):
        ...
        while True:
            ...
            if record_type == expected_type:
                items = getattr(pb, expected_type, None)
                for item in items.update:
                    k, v = item.key, item.value_json
                    if k in required_keys:
                        new_k = k.split("/")[-1]
                        ret_dict[new_k] = ast.literal_eval(v)
                found = True
                break
    ...
    def get_opt_state(self):
        with tempfile.TemporaryDirectory() as tmp_dir:  # avoid multiple artifact copies
            ...
            if self.restore_state is True and os.path.isdir(self.model_name_or_path):
                self.restore_state = str(
                    Path(self.model_name_or_path) / "opt_state.msgpack"
                )
            elif self.restore_state is True and "/model-" in self.model_name_or_path:
                # wandb artifact
...
class TrainingArguments:
    ...
    lr_decay: str = field(
        default=None,
        metadata={
            "help": "Decay to be used in the learning rate scheduler. Can be None (default), linear or exponential or piecewise."
        },
    )
    lr_decay_steps: int = field(
        default=0,
        metadata={
            "help": "Number of steps to decay the learning rate (used in linear decay)."
        },
    )
    lr_decay_till_zero: Optional[bool] = field(
        default=False,
        metadata={
            "help": "The final piece of a piecewise linear scheduler which eventually reachs to zero learning rate."
        },
    )
    lr_interpolate_type: str = field(
        default="linear",
        metadata={
            "help": "Decay to be used in the learning rate scheduler. Can be linear (default) or cosine."
        },
    )
    lr_boundaries_and_scales: Optional[ast.literal_eval] = field(
        default=str(dict()),
        metadata={
            "help": "See https://optax.readthedocs.io/en/latest/api.html#optax.piecewise_interpolate_schedule."
        },
    )
    lr_final: float = field(
        default=None,
        metadata={"help": "The final learning rate."},
    )
    ...
    eval_epoch: bool = field(
        default=True, metadata={"help": "Run an evaluation after every epoch."}
    )
    eval_final: bool = field(
        default=True, metadata={"help": "Run an evaluation after final epoch."}
    )
    save_steps: int = field(
        default=4000, metadata={"help": "Save checkpoint every X updates steps."}
    )
    save_epoch: bool = field(
        default=True, metadata={"help": "Save checkpoint after every epoch."}
    )
    save_final: bool = field(
        default=True, metadata={"help": "Save checkpoint after final epoch."}
    )
...
def main():
    ...
    def create_learning_rate_fn() -> Callable[[int], jnp.array]:
        ...
        if training_args.lr_decay is None:
            ...
            transition_steps = (
                ...
                if training_args.lr_decay_steps > 0
                else num_train_steps - training_args.warmup_steps
            )  # FIXME is offset included in num_train_steps? seems so! let's assume offset always 0
            #  # No, state.step may be overrided by an existing checkpoint; it does not count from 0
            end_value = (
                0.0 if training_args.lr_final is None else training_args.lr_final
            )
            ...
            if (
                end_value != 0.0
                and training_args.lr_decay_steps > 0
                and training_args.lr_decay_till_zero
                and steps_till_end > 0
            ):
                final_fn = optax.linear_schedule(
                    init_value=end_value,
                    end_value=0,
                    transition_steps=steps_till_end,
                )
                decay_fn = optax.join_schedules(
                    schedules=[decay_fn, final_fn],
                    boundaries=[training_args.lr_decay_steps],
                )
            ...
            decay_fn = optax.exponential_decay(
                ...
                end_value=training_args.lr_final,
            )
        elif training_args.lr_decay == "piecewise":
            decay_fn = optax.piecewise_interpolate_schedule(
                training_args.lr_interpolate_type,
                training_args.learning_rate,
                training_args.lr_boundaries_and_scales,
            )
    ...
    with mesh:
        state_info = "warm" if model_args.restore_state else "cold"
        logger.info(f"  Creating {state_info} state")
        ...
        logger.info(f"  Initial state: {attr_state}")
    ...
    initial_epoch = local_state["epoch"]
    epochs = tqdm(
        range(initial_epoch, num_epochs),
        desc=f"Epoch... ({initial_epoch + 1}/{num_epochs})",
        position=0,
    ...
    with mesh:
        for epoch in epochs:
            ...
            if epoch > initial_epoch and data_args.do_reprocessing:
                logger.info(f"  Reprocess train data #epoch {epoch + 1}")
            ...
            if not evaluation_ran and (  # let's save some time
                training_args.eval_epoch
                or (
                    training_args.eval_final
